...
exports.indicateIntegrity = function(procurement, contracts) {
	...
	indicators.multipleBidders = procurement.bidder_count != 1

	indicators.costAndEstimation = procurement.cost != null ? (
		procurement.cost <= procurement.estimated_cost * 1.1
	) : null

	indicators.disputes = procurement.dispute_count == 0

	indicators.sellerDonations = contracts ? (
		contracts.every((contract) => contract.donations.length == 0)
	) : null
	...
	if (procurement.deadline_at) {
		...
		indicators.biddingPeriod = !(
			biddingDurationInDays >= 0 && biddingDurationInDays <= 32 ||
			biddingDurationInDays >= 50 && biddingDurationInDays <= 57
		)
	}
	else indicators.biddingPeriod = null
